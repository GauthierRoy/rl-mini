...
- #26 PII redaction/extraction as flagship (DEVELOPED Sep 2026, see spec below): verifiable reward = entity F1 + schema validity + brevity; synthetic PII notes generatable same pattern as countdown (reverse-gen solvable entities, bias-controlled demographics). Direct leverage: working on PII models at work right now — domain data intuition + real-world framing for free. Design notes (brainstorm): CSV as output schema (compact, fast to parse/verify vs JSON); multi-task via input task tokens (one model, `<EXTRACT>`/`<REDACT>`/... prefixes); small encoder backbone for inference speed — CORRECTION: no ModernBERT-small exists (base 149M `answerdotai/ModernBERT-base` / large 395M only); "mmbert" = confirm what user meant (mBERT? MiniLM? internal?). Alternatives: MiniLM-L6 (91MB, Redact proj hit 0.9247 F1 on 5 labels) or ModernBERT-base. Synthetic-PII-generation-in-the-loop à la OpenMed training, mixed with RLVR.
- #26 spec: decoder track = Qwen3.5-0.8B extraction-as-generation (CSV rows) + RLVR, countdown skills transfer 1:1; encoder track = MiniLM/ModernBERT token-classification SFT baseline (cheap, fast, strong). Headline comparison: RLVR-vs-SFT for structured extraction + beating GPT-4o zero-shot (0.544 F1 on AI4Privacy, very beatable). Data: own synthetic generator for train (clean IP) + AI4Privacy (pii-masking-300k: OpenPII-220k 27 classes + FinPII-80k; check license.md before publishing) + SPY (NAACL'25, LLM-generated med/legal) for OOD eval. Start narrow: 5-8 high-signal entity types (Redact lesson: cutting 13→5 labels jumped F1 0.56→0.92). No work data ever — synthetic only.
...
- V2 switched to correctness-only (no format reward): scan free text for candidate equations, tiered `grade_equation` (2.0 exact / 0.3 exact-numbers-wrong-value / 0.1 parses-with-allowed-numbers). Rationale: SimpleRL-Zoo strict-format-hurts-exploration + own logs show enumeration behavior; echo-hack stays dead via out-of-range example. Non-thinking runs wire only this func. Hardened Sep 2026: 0.1 tier needs an operator (kills `2=2` spam), LaTeX normalized (`\times`/`$`) before eval, paren-group candidates catch prose-embedded equations, example moved to `(30-18)*2` so every echo fragment scores 0.
- V3 = SFT cold-start + correctness-only RL (Sep 2026): `find_equation` brute-force solver (verified 50/50) → 200 bare-equation SFT traces → merged fp model → RL from merged path (`configs/sft_format_08b.yaml`, `configs/countdown_q35_08b_v3.yaml`, `src/sft_format.py`). `train_countdown.py` template patch triggers on local-dir model paths too.
